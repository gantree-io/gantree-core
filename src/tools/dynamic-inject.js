...
function _realInject(chainSpec, validatorSpec) {
  let runtimeObj = _insertKeys(chainSpec.genesis.runtime, validatorSpec)
...
function createRuntime(baseRuntime, validatorSpec, palletSpec) {
  const tokenizedPalletSpec = tokenReplacer({ palletSpec, tokens: validatorSpec.validators })
